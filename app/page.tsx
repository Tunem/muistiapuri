...
interface Goal { id: string; title: string; description: string; target: number; current: number; unit: string; color: "pine"|"clay"|"gold"; createdAt: string; }
...
const inp: CSSProperties = { border: "1px solid var(--border)", borderRadius: 8, padding: "10px 12px", fontSize: 13, background: "var(--bg)", outline: "none", color: "var(--text)" };

/* ── DayView ──────────────────────────────────────────── */
function DayView() {
  const [tasks, setTasks] = useLS<Task[]>("app-tasks", []);
  const [input, setInput] = useState("");
  const [prio, setPrio] = useState<Task["priority"]>("normaali");
  const [showDone, setShowDone] = useState(false);
  const td = today();
  const pending = tasks.filter(t => t.date === td && !t.done);
  const done = tasks.filter(t => t.date === td && t.done);
  const h = new Date().getHours();
  const greeting = h < 12 ? "Hyvää huomenta" : h < 18 ? "Hyvää päivää" : "Hyvää iltaa";

  function add() {
    if (!input.trim()) return;
    setTasks(ts => [...ts, { id: uid(), text: input.trim(), done: false, priority: prio, date: td }]);
    setInput("");
  }
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {(["matala","normaali","korkea"] as Task["priority"][]).map(p => (
            <button key={p} onClick={() => setPrio(p)} style={{
              padding: "4px 12px", borderRadius: 20, border: `1px solid ${prio === p ? "transparent" : "var(--border)"}`,
              background: prio === p ? prioBg[p] : "transparent", color: prio === p ? prioColor[p] : "var(--muted)",
              cursor: "pointer", fontSize: 11, fontWeight: prio === p ? 700 : 300
            }}>{p}</button>
          ))}
        </div>
      </div>
...
function CalendarView() {
  const [events, setEvents] = useLS<CalEvent[]>("app-events", []);
  ...
  function addEvent() {
    ...
    setEvents(es=>[...es,{id:uid(),title:form.title.trim(),date:selected,time:form.time,color:form.color}]);
    setForm({title:"",time:"09:00",color:"pine"});
    setShowForm(false);
  }
  ...
  const hasDot = new Set(events.filter(e=>e.date.startsWith(monthPfx)).map(e=>e.date));
          ...
          {cells.map((d,i)=>{
            ...
            return (
              <button key={date} onClick={()=>setSelected(date)} style={{
                ...
                background: isSel?"var(--pine)":isTd?"var(--pine-light)":"transparent",
                color: isSel?"#fff":isTd?"var(--pine)":"var(--text)",
                fontSize:13, fontWeight:(isSel||isTd)?700:300, position:"relative", transition:"all 0.15s"
              }}>
                {d}
            ...
            : <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                {selEvents.map(ev=>(
                  <div key={ev.id} style={{ display:"flex", alignItems:"flex-start", gap:10, padding:"10px 12px", borderRadius:8, borderLeft:`3px solid ${dot[ev.color]}`, background:"var(--bg)" }}>
                    ...
                    <button onClick={()=>setEvents(es=>es.filter(e=>e.id!==ev.id))} style={{ background:"none", border:"none", color:"var(--faint)", cursor:"pointer", fontSize:16 }}>×</button>
                  </div>
                ))}
              </div>
          }
...
/* ── GoalsView ────────────────────────────────────────── */
function GoalsView() {
  const [goals, setGoals] = useLS<Goal[]>("app-goals", []);
  const [showForm, setShowForm] = useState(false);
  ...
  function addGoal() {
    ...
    setGoals(gs=>[...gs,{id:uid(),title:form.title.trim(),description:form.description.trim(),target,current:0,unit:form.unit.trim()||"yksikköä",color:form.color,createdAt:today()}]);
    setForm({title:"",description:"",target:"",unit:"",color:"pine"});
    setShowForm(false);
  }

  function adjust(id: string, delta: number) { setGoals(gs=>gs.map(g=>g.id===id?{...g,current:Math.max(0,Math.min(g.target,g.current+delta))}:g)); }
    ...
    <div style={{ maxWidth:660, margin:"0 auto" }}>
      ...
      {goals.length===0&&(
        <div style={{ textAlign:"center", padding:"60px 0", color:"var(--muted)" }}>
          ...
          <p style={{ fontSize:15, marginBottom:6 }} className="serif">Aseta ensimmäinen tavoitteesi</p>
            ...
            <div key={g.id} style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:14, padding:22 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  ...
                  <button onClick={()=>setGoals(gs=>gs.filter(x=>x.id!==g.id))} style={{ background:"none", border:"none", color:"var(--faint)", cursor:"pointer", fontSize:16 }}>×</button>
                </div>
              </div>
  ...
  return (
    <div style={{ minHeight:"100vh" }}>
      ...
      <main style={{ padding:"36px 40px" }}>
        {tab==="paiva"&&<DayView/>}
        {tab==="kalenteri"&&<CalendarView/>}
        {tab==="tavoitteet"&&<GoalsView/>}
      </main>
    </div>
  );
}
